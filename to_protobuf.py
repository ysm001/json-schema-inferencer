...
def to_protobuf(name, schema):
    ...
    return 'syntax = "proto3";\n\nimport "google/protobuf/any.proto";\n\n{}'.format(stringify_message_def(parsed_object))
...
def parse_anyof(name, schema):
    ...
    if len(schema['anyOf']) != 2 or 'null' not in types:
        raise Exception('anyOf is only allowed to be used for representing nullable. [{}]'.format(', '.join(types)))
    ...
    # return {
    #     'type': 'oneof',
    #     'name': 'oneof_{}'.format(hashlib.sha256(json.dumps(schema).encode()).hexdigest()[0:8]),
    #     'items': [{**{'tag': i}, **parse_field('{}-{}'.format(name, i), s)} for i, s in enumerate(schema['anyOf'])]
    # }

def parse_object(name, schema):
    properties = schema['properties'].items()
    fields = [{**{'tag': i + 1, 'is_required': k in schema['required']}, **parse_field(k, v)} for i, (k, v) in enumerate(properties)]
    return {
        'name': name,
        'type': 'message',
        'fields': fields
...
def stringify_field(proto):
    ...
    # return 'oneof {}_optional {{\n{}\n}}'.format(proto['name'], body)
...
def stringify_message_def(proto):
    ...
    return 'message {} {{\n{}{}\n}}'.format(
        message_name,
        extract_message_defs(proto['fields']),
        '\n'.join(field_strs)
    )
...
def stringify_array(proto):
    ...
    return '{}repeated {}'.format(
        extract_message_defs([items]),
        stringify({**items, **{'tag': proto['tag']}}),
    )
...
def stringify_oneof(proto):
    ...
    return 'oneof {} {{\n{}{}\n}}'.format(
        proto['name'],
        extract_message_defs(proto['items']),
        '\n'.join(field_strs)
    )
